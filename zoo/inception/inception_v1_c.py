...
class InceptionV1(Composable):
    ...
    def inception_block(x, f1x1, f3x3, f5x5, fpool, **metaparameters):
        ...
        """
        # 1x1 branch
        b1x1 = Composable.Conv2D(x, f1x1[0], (1, 1), strides=1, padding='same', **metaparameters)
        b1x1 = Composable.ReLU(b1x1)

        # 3x3 branch
        # 3x3 reduction
        b3x3 = Composable.Conv2D(x, f3x3[0], (1, 1), strides=1, padding='same', **metaparameters)
        b3x3 = Composable.ReLU(b3x3)
        b3x3 = ZeroPadding2D((1,1))(b3x3)
        b3x3 = Composable.Conv2D(b3x3, f3x3[1], (3, 3), strides=1, padding='valid', **metaparameters)
        b3x3 = Composable.ReLU(b3x3)

        # 5x5 branch
        # 5x5 reduction
        b5x5 = Composable.Conv2D(x, f5x5[0], (1, 1), strides=1, padding='same', **metaparameters)
        b5x5 = Composable.ReLU(b5x5)
        b5x5 = ZeroPadding2D((1,1))(b5x5)
        b5x5 = Composable.Conv2D(b5x5, f5x5[1], (3, 3), strides=1, padding='valid', **metaparameters)
        b5x5 = Composable.ReLU(b5x5)

        # Pooling branch
        bpool = MaxPooling2D((3, 3), strides=1, padding='same')(x)
        # 1x1 projection
        bpool = Composable.Conv2D(bpool, fpool[0], (1, 1), strides=1, padding='same', **metaparameters)
