...
def stem(inputs, alpha):
    ...
    # Convolutional block
    x = Conv2D(n_filters, (3, 3), strides=(2, 2), padding='same', kernel_initializer='glorot_uniform')(inputs)
...
def HS(x):
    """ Construct Hard-Swish Activation
        x : input to tensor
    """
    x = (x * K.relu(x + 3, max_value=6.0)) / 6.0
    return x
